            ...
            Exception
    {
        ...
        if (cust == null)
        {
            new Event(Event.getLeafLevelClassName(this), "<init>",
                    "Missing patron holder information", Event.FATAL);
            throw new Exception
                    ("UNEXPECTED ERROR: PatronCollection.<init>: patron holder information is null");
        }

        String patronHolderId = (String)cust.getState("ID");

        if (patronHolderId == null)
        {
            new Event(Event.getLeafLevelClassName(this), "<init>",
                    "Data corrupted: Patron Holder has no id in database", Event.FATAL);
            throw new Exception
                    ("UNEXPECTED ERROR: PatronCollection.<init>: Data corrupted: patron holder has no id in repository");
        }

        String query = "SELECT * FROM " + myTableName + " WHERE (OwnerID = " + patronHolderId + ")";
        ...
        else
        {
            throw new InvalidPrimaryKeyException("No patrons for customer : "
                    + patronHolderId + ". Name : " + cust.getState("Name"));
        }
